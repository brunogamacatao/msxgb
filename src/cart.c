...
bool cart_load(c8 *filename) {
  ...
  for (i = 0; i < ctx.num_segments; i++) {
    ...
    if (!DOSMapper_Alloc(DOS_ALLOC_USER,
                         DOS_SEGSLOT_PRIM | DOS_SEGSLOT_OTHERFIRST,
                         &ctx.segments[i])) {
      printf("Error: failed to allocate segment %d\r\n", i);
      DOS_FClose(fp);
      return false;
    }
    printf("Segment %d allocated (slot=%d num=%d)\r\n", i, ctx.segments[i].Slot,
           ctx.segments[i].Number);
  }
...
// DOSMapper_ReadByte switches the segment internally — no manual page mapping
// needed.
