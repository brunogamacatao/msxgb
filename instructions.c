...
void inst_to_str(cpu_context *ctx, char *str) {
  ...
  case AM_R_D16:
  case AM_R_A16:
    sprintf(str, "%s %s,$%04X", inst_name(inst->type), rt_lookup[inst->reg_1],
            ctx->fetched_data);
    return;
  ...
  case AM_R_R:
    sprintf(str, "%s %s,%s", inst_name(inst->type), rt_lookup[inst->reg_1],
  ...
  case AM_R_D8:
  case AM_R_A8:
    sprintf(str, "%s %s,$%02X", inst_name(inst->type), rt_lookup[inst->reg_1],
  ...
  case AM_HL_SPR:
    sprintf(str, "%s (%s),SP+%d", inst_name(inst->type), rt_lookup[inst->reg_1],
            ctx->fetched_data & 0xFF);
    return;
  ...
  case AM_A16_R:
    sprintf(str, "%s ($%04X),%s", inst_name(inst->type), ctx->fetched_data,
            rt_lookup[inst->reg_2]);
    return;
